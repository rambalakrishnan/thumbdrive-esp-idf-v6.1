...
check_config() {
    ...
    local actual
    actual=$(grep "^${key}=" "${PROJECT_DIR}/sdkconfig.defaults" 2>/dev/null | tail -1 | cut -d= -f2-) || true
...
check_config_absent() {
    ...
    local line
    line=$(grep "^${key}=" "${PROJECT_DIR}/sdkconfig.defaults" 2>/dev/null | head -1) || true
...
test_source_files() {
    ...
    for f in "${PROJECT_DIR}/main/main.c" "${PROJECT_DIR}/partitions.csv" \
        ...
        if [ -f "$f" ]; then
            count=$(grep -cP '[^\x00-\x7f]' "$f" 2>/dev/null || true)
...
test_sdkconfig_defaults() {
    ...
    # CONFIG_WL_SECTOR_SIZE=512 is generated by Kconfig in the build's sdkconfig,
    # not directly in sdkconfig.defaults. Check the generated config instead.
    TOTAL=$((TOTAL + 1))
    local gen_config="${PROJECT_DIR}/sdkconfig"
    local wl_sector
    wl_sector=$(grep "^CONFIG_WL_SECTOR_SIZE=" "$gen_config" 2>/dev/null | tail -1 | cut -d= -f2-) || true
    if [ "$wl_sector" = "512" ]; then
        echo -e "  ${GREEN}[PASS]${NC} Generated sdkconfig has CONFIG_WL_SECTOR_SIZE=512"
        PASS=$((PASS + 1))
    else
        echo -e "  ${RED}[FAIL]${NC} Generated sdkconfig CONFIG_WL_SECTOR_SIZE should be 512, got: ${wl_sector:-<not set>}"
        FAIL=$((FAIL + 1))
    fi
...
test_partitions() {
    ...
    TOTAL=$((TOTAL + 1))
    local nvs_offset
    nvs_offset=$(grep -i '^nvs,' "$csv" | awk -F, '{print $4}' | head -1 | tr -d ' ')
    ...
    TOTAL=$((TOTAL + 1))
    local nvs_size
    nvs_size=$(grep -i '^nvs,' "$csv" | awk -F, '{print $5}' | head -1 | tr -d ' ')
    ...
    TOTAL=$((TOTAL + 1))
    local phy_offset
    phy_offset=$(grep -i '^phy_init,' "$csv" | awk -F, '{print $4}' | head -1 | tr -d ' ')
    ...
    factory_offset=$(grep -i '^factory,' "$csv" | awk -F, '{print $4}' | head -1 | tr -d ' ')
    ...
    TOTAL=$((TOTAL + 1))
    local factory_size
    factory_size=$(grep -i '^factory,' "$csv" | awk -F, '{print $5}' | head -1 | tr -d ' ')
    ...
    storage_offset=$(grep -i '^storage,' "$csv" | awk -F, '{print $4}' | head -1 | tr -d ' ')
    ...
    TOTAL=$((TOTAL + 1))
    local storage_size
    storage_size=$(grep -i '^storage,' "$csv" | awk -F, '{print $5}' | head -1 | tr -d ' ')
    ...
    local storage_end=$(( 0x40000 + 0xfc0000 ))
    if [ $((storage_end)) -eq $((0x1000000)) ]; then
    ...
    local factory_type
    factory_type=$(grep -i '^factory,' "$csv" | awk -F, '{print $2}' | head -1 | tr -d ' ') || true
    local storage_type
    storage_type=$(grep -i '^storage,' "$csv" | awk -F, '{print $3}' | head -1 | tr -d ' ') || true
...
test_main_c() {
    ...
    TOTAL=$((TOTAL + 1))
    # Verify MOUNT_APP is NOT used as a mount_point VALUE in code (only in comments is OK)
    # We check for "MOUNT_APP" as a standalone token in non-comment lines
    local app_usages
    # Strip comment lines (starting with // or * or #), then check for MOUNT_APP
    app_usages=$(grep -v '^\s*//' "${PROJECT_DIR}/main/main.c" | grep -v '^\s*\*' | grep -v '^\s*#' | grep -c "MOUNT_APP" 2>/dev/null || true)
    app_usages=$(echo "$app_usages" | tr -cd '0-9')
    app_usages=${app_usages:-0}
    if [ "$app_usages" -eq 0 ]; then
        echo -e "  ${GREEN}[PASS]${NC} MOUNT_APP not used in code (only in comments — acceptable)"
        PASS=$((PASS + 1))
    else
        echo -e "  ${RED}[FAIL]${NC} MOUNT_APP found in $app_usages code line(s) — would trigger FAT mount"
        FAIL=$((FAIL + 1))
    fi
...
test_build() {
    ...
    if [ ! -f "$gen_sdkconfig" ]; then
        gen_sdkconfig="${PROJECT_DIR}/sdkconfig"
    fi
    if [ -f "$gen_sdkconfig" ]; then
...
test_flash_command() {
    ...
    if [ -f "$flash_args" ]; then
        ...
        has_flash_size=$(grep -c "16MB" "$flash_args" 2>/dev/null || true)
        has_offset_0=$(grep -c "0x0" "$flash_args" 2>/dev/null || true)
        has_offset_8000=$(grep -c "0x8000" "$flash_args" 2>/dev/null || true)
        has_offset_10000=$(grep -c "0x10000" "$flash_args" 2>/dev/null || true)
        has_flash_size=${has_flash_size:-0}
        has_offset_0=${has_offset_0:-0}
        has_offset_8000=${has_offset_8000:-0}
        has_offset_10000=${has_offset_10000:-0}
        # Normalize — strip any whitespace/newlines
        has_flash_size=$(echo "$has_flash_size" | tr -cd '0-9'); has_flash_size=${has_flash_size:-0}
        has_offset_0=$(echo "$has_offset_0" | tr -cd '0-9'); has_offset_0=${has_offset_0:-0}
        has_offset_8000=$(echo "$has_offset_8000" | tr -cd '0-9'); has_offset_8000=${has_offset_8000:-0}
        has_offset_10000=$(echo "$has_offset_10000" | tr -cd '0-9'); has_offset_10000=${has_offset_10000:-0}
    ...
    if [ -d "$gen_pt_dump" ]; then
        ...
        if [ -f "$gen_script" ]; then
            ...
            has_nvs=$(echo "$pt_dump" | grep -c "^nvs," 2>/dev/null || true)
            has_factory=$(echo "$pt_dump" | grep -c "^factory," 2>/dev/null || true)
            has_storage=$(echo "$pt_dump" | grep -c "^storage," 2>/dev/null || true)
            # Normalize to integers (grep -c returns "0\n0" on failure)
            has_nvs=$(echo "$has_nvs" | tr -cd '0-9'); has_nvs=${has_nvs:-0}
            has_factory=$(echo "$has_factory" | tr -cd '0-9'); has_factory=${has_factory:-0}
            has_storage=$(echo "$has_storage" | tr -cd '0-9'); has_storage=${has_storage:-0}
...
test_github() {
    ...
    # NOTE: Use grep (not grep -q) in pipeline to avoid SIGPIPE with pipefail,
    # which causes git log to exit 141 when grep -q closes the pipe early.
    local git_log_output
    git_log_output=$(cd "$PROJECT_DIR" && git log --oneline 2>/dev/null)
    if echo "$git_log_output" | grep -q "MOUNT_USB"; then
...
test_binary_inspection() {
    ...
    if [ -f "${BUILD_DIR}/combined.bin" ]; then
        ...
        # ESP32-S3 image header magic byte: 0xE9 (extended) or 0x07 (legacy)
        # The combined.bin starts with bootloader.bin, which uses 0xE9 on ESP32-S3
        local first_byte
        first_byte=$(xxd -l 1 -p "${BUILD_DIR}/combined.bin" 2>/dev/null)
        if echo "$first_byte" | grep -qiE "^e9|^07"; then
            echo -e "  ${GREEN}[PASS]${NC} combined.bin starts with valid ESP32 image header (magic 0x${first_byte})"
            PASS=$((PASS + 1))
